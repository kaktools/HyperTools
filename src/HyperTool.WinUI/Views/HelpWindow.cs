...
public sealed class HelpWindow : Window
{
    ...
    private UIElement BuildLayout()
    {
        ...
        bodyStack.Children.Add(CreateSection("VM Aktionen", "Start, Stop, Hard Off, Restart und Konsole stehen als Schnellaktionen bereit (inklusive Tray-Aktionen)."));
        bodyStack.Children.Add(CreateSection("VMConnect Hostname", "Im Konfig-Bereich kannst du den VMConnect-Hostnamen mit einem Klick auf den empfohlenen Rechnernamen setzen. Das reduziert Zertifikatswarnungen beim Öffnen der Konsole."));
        ...
        bodyStack.Children.Add(CreateSection("Netzwerk", "Switches werden pro VM-Adapter verwaltet (auch Multi-NIC). Im VM-Bereich kannst du Adapter direkt erstellen, bearbeiten, löschen und pro Adapter den Switch gezielt setzen."));
        bodyStack.Children.Add(CreateSection("Netzwerk-Switch Performance", "Bei vielen Hyper-V-VMs wurden Laden, Umschalten und Aktualisieren im Netzwerkbereich beschleunigt. Adapter- und Switch-Status werden schneller nachgezogen."));
        bodyStack.Children.Add(CreateSection("Switch-Verwaltung", "In den Einstellungen kannst du Hyper-V-Switches erstellen, bearbeiten und löschen. Bei External-Änderungen mit Adapterwechsel nutzt HyperTool einen sicheren Recreate-Flow mit Bestätigungsdialog."));
        bodyStack.Children.Add(CreateSection("USB-Share (Host)", "USB Share zeigt oben rechts einen Aktiv/Inaktiv-Status-Chip. Bei deaktiviertem Feature erscheint im Tabellenbereich ein zentrierter Hinweis 'Deaktiviert'; nach Reaktivierung wird nach ca. 1 Sekunde automatisch aktualisiert. Verwaiste Shared-Einträge ohne angestecktes Gerät werden in der Host-Liste ausgeblendet."));
        bodyStack.Children.Add(CreateSection("USB-Share Synchronisierung", "Share-, Unshare- und Detach-Aktionen werden robuster zwischen Host und Guest abgeglichen. Dadurch bleiben Gerätestatus und Anzeige zwischen Hauptfenster und Tray konsistenter."));
        bodyStack.Children.Add(CreateSection("Optionale Laufzeiten", "Fehlende Host-Laufzeiten (z. B. usbipd-win) lassen sich direkt aus der App nachinstallieren. Die Installation läuft still und nutzt bei Bedarf Fallback-Aufrufe, damit sie zuverlässiger abgeschlossen wird."));
        ...
        bodyStack.Children.Add(CreateSection("Tray Netzwerk-Sync", "Switch-Änderungen aus VM-Menü, Netzwerkbereich und Tray werden wieder konsistenter synchronisiert, damit aktive Adapter/Switches überall denselben Stand zeigen."));
        ...
        bodyStack.Children.Add(CreateSection("Startverhalten", "Netzwerkdaten (Switches + VM-Adapterzustand) werden beim Start direkt geladen. Nach Adapter-Aktionen wird die Anzeige sofort aktualisiert, ohne dass du die VM neu auswählen musst."));
