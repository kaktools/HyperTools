...
public sealed class MainWindow : Window
{
    ...
    private const string GuestRuntimeOwner = "kaktools";
    ...
    private RowDefinition? _headerToNoticeSpacerRow;
    private RowDefinition? _noticeToContentSpacerRow;
    ...
    private Border BuildThemeTransitionOverlay()
    {
        ...
        overlayGrid.Children.Add(new Rectangle
        {
            ...
            Fill = new SolidColorBrush(Windows.UI.Color.FromArgb(0x66, 0x03, 0x0A, 0x18))
        });
    ...
    private UIElement BuildLayout()
    {
        ...
        _headerToNoticeSpacerRow = new RowDefinition { Height = new GridLength(12) };
        root.RowDefinitions.Add(_headerToNoticeSpacerRow);
        root.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        _noticeToContentSpacerRow = new RowDefinition { Height = new GridLength(12) };
        root.RowDefinitions.Add(_noticeToContentSpacerRow);
        ...
        mainGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(106) });
        ...
        contentGrid.RowDefinitions.Add(new RowDefinition { Height = new GridLength(6) });
        ...
        var bottomCard = CreateCard(new Thickness(16, 0, 16, 16), 14, 12);
    ...
    private UIElement BuildVmPage()
    {
        ...
        _mountIsoButton = CreateIconButton("💿", "ISO einbinden", onClick: async (_, _) =>
        {
            ...
            {
                await MountIsoForVmAsync(_viewModel.SelectedVm);
            }
        });
        actionRow2.Children.Add(_mountIsoButton);
        _unmountIsoButton = CreateIconButton("⏏", "ISO auswerfen", onClick: async (_, _) =>
        {
            if (_viewModel.SelectedVm is not null)
            {
                await UnmountIsoForVmAsync(_viewModel.SelectedVm);
            }
        });
        actionRow2.Children.Add(_unmountIsoButton);
    ...
    private UIElement BuildInfoPage()
    {
        ...
        var guestDownloadButton = CreateIconButton("⬇", "HyperTool Guest", onClick: async (_, _) => await OpenLatestGuestReleasePageAsync());
        ...
        ToolTipService.SetToolTip(guestDownloadButton, "Öffnet die neueste HyperTool Guest Release-Seite auf GitHub. Download und Installation erfolgen dort manuell.");
    ...
    private UIElement BuildUsbPage()
    {
        ...
        _usbDevicesListView.ContainerContentChanging += OnUsbDevicesListContainerContentChanging;
        ...
        var metadataHeader = new TextBlock
        {
            Text = "Kommentar",
            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
            Opacity = 0.9,
    ...
    private void OnUsbDevicesListContainerContentChanging(ListViewBase sender, ContainerContentChangingEventArgs args)
    {
        ...
        var commentBox = FindDescendantByName<TextBox>(templateRoot, "UsbInlineCommentBox");
        if (commentBox is null)
        {
            return;
        }
    ...
    {
        if (sender is TextBox commentBox)
        {
            SaveUsbInlineComment(commentBox);
        }
    }

    private void SaveUsbInlineComment(TextBox commentBox)
    {
        if (commentBox.DataContext is not UsbIpDeviceInfo device)
        {
            return;
        }
...
""";
    ...
    private async Task RunHostSharedFolderSelfTestAsync()
    {
        ...
        try
        {
            ...
            var registryServiceOk = App.AreAllRequiredHyperVSocketServicesRegistered();
    ...
    private static NumberBox CreateStyledNumberBox(double value, double minValue, double maxValue)
    {
    ...
    {
        var hasNotice = _viewModel.HasConfigurationNotice;
        _configurationNoticeText.Text = _viewModel.ConfigurationNotice ?? string.Empty;
        _configurationNoticeBorder.Visibility = hasNotice ? Visibility.Visible : Visibility.Collapsed;

        // Keep vertical rhythm consistent: when notice is hidden we keep only one spacer between header and content.
        if (_headerToNoticeSpacerRow is not null)
        {
            _headerToNoticeSpacerRow.Height = hasNotice ? new GridLength(12) : new GridLength(0);
        }

        if (_noticeToContentSpacerRow is not null)
        {
            _noticeToContentSpacerRow.Height = new GridLength(12);
        }
    }
    ...
    private void RefreshVmAdapterCards()
    {
        ...
        foreach (var adapter in _viewModel.AvailableVmNetworkAdapters)
        {
            ...
            cardStack.Children.Add(new TextBlock
            {
                Text = adapter.GuestNetworkDisplay,
                Opacity = 0.78,
                TextWrapping = TextWrapping.Wrap
            });
    ...
    private Button CreateVmChip(VmDefinition vm)
    {
        ...
        {
            Text = string.Equals(vm.MonitorStateText, "Connected", StringComparison.OrdinalIgnoreCase)
                ? $"{vm.MonitorCpuText} · {vm.MonitorRamText}"
                : vm.MonitorStateText,
            TextTrimming = TextTrimming.CharacterEllipsis,
            Opacity = 0.78,
            FontSize = 11,
        ...
        textStack.Children.Add(monitorText);
    ...
    private async Task RenameSelectedVmWithPromptAsync()
    {
        ...
        await RenameVmWithPromptAsync(_viewModel.SelectedVm);
    }

    private async Task EditSelectedVmComputeResourcesAsync()
    {
        if (_viewModel.SelectedVm is null)
        {
            return;
    ...
    private async Task RenameVmWithPromptAsync(VmDefinition vm)
    {
        ...
        var nameBox = CreateStyledTextBox(vm.Name, "Neuer VM-Name");
        nameBox.Text = vm.Name;

        var content = new StackPanel { Spacing = 8 };
        content.Children.Add(new TextBlock
        {
            Text = $"VM '{vm.Name}' umbenennen",
            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
            TextWrapping = TextWrapping.Wrap
        });
        content.Children.Add(nameBox);
        ...
        var requestedName = (nameBox.Text ?? string.Empty).Trim();
        if (string.IsNullOrWhiteSpace(requestedName))
        {
            return;
        }

        var renamed = await _viewModel.RenameVmByNameAsync(vm.Name, requestedName);
        if (renamed)
        {
            RequestVmChipsRefresh();
        }
    ...
    private async Task EditVmComputeResourcesWithPromptAsync(VmDefinition vm)
    {
        if (vm is null || string.IsNullOrWhiteSpace(vm.Name))
        {
            return;
        ...
        if (!IsVmStoppedForComputeChange(vm.RuntimeState))
        {
            _viewModel.PublishNotification(
                $"CPU/RAM kann nur geändert werden, wenn die VM gestoppt ist. Aktueller Status: {vm.RuntimeState}.",
                "Warning");
            return;
        }
        ...
        var settings = await _viewModel.GetVmComputeSettingsAsync(vm.Name);
        if (settings is null)
        {
            return;
        }

        var primaryTextBrush = Application.Current.Resources["TextPrimaryBrush"] as Brush;
        var mutedTextBrush = Application.Current.Resources["TextMutedBrush"] as Brush;
        var panelBackgroundBrush = Application.Current.Resources["PanelBackgroundBrush"] as Brush;
        var panelBorderBrush = Application.Current.Resources["PanelBorderBrush"] as Brush;
        var surfaceSoftBrush = Application.Current.Resources["SurfaceSoftBrush"] as Brush;

        var cpuNumberBox = CreateStyledNumberBox(settings.CpuCount, settings.MinCpuCount, settings.MaxCpuCount);
        var ramNumberBox = CreateStyledNumberBox(settings.StartupMemoryGb, settings.MinStartupMemoryGb, settings.MaxStartupMemoryGb);

        var cpuSlider = new Slider
        {
            Minimum = settings.MinCpuCount,
            Maximum = settings.MaxCpuCount,
            StepFrequency = 1,
            SmallChange = 1,
            LargeChange = 1,
            Value = settings.CpuCount,
            MinWidth = 0,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment = VerticalAlignment.Center
        };
        ...
        {
            Minimum = settings.MinStartupMemoryGb,
            Maximum = settings.MaxStartupMemoryGb,
            StepFrequency = 1,
            SmallChange = 1,
            LargeChange = 1,
            Value = settings.StartupMemoryGb,
            MinWidth = 0,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment = VerticalAlignment.Center
        };

        cpuNumberBox.HorizontalAlignment = HorizontalAlignment.Stretch;
        ramNumberBox.HorizontalAlignment = HorizontalAlignment.Stretch;

        var cpuHint = new TextBlock { Opacity = 0.8, FontSize = 12 };
        var ramHint = new TextBlock { Opacity = 0.8, FontSize = 12 };
        cpuHint.Foreground = mutedTextBrush;
        ramHint.Foreground = mutedTextBrush;

        var syncInProgress = false;
        void SyncHints()
        {
            cpuHint.Text = $"{(int)cpuSlider.Value} vCPU   (Host: max. {settings.HostLogicalProcessorCount})";
            ramHint.Text = $"{(int)ramSlider.Value} GB Startup-RAM   (Host gesamt: {settings.HostTotalMemoryGb} GB)";
        }

        cpuSlider.ValueChanged += (_, _) =>
        {
            if (syncInProgress)
            {
                return;
            ...
            syncInProgress = true;
            cpuNumberBox.Value = Math.Round(cpuSlider.Value);
            SyncHints();
            syncInProgress = false;
        };

        ramSlider.ValueChanged += (_, _) =>
        {
            if (syncInProgress)
            {
                return;
            ...
            syncInProgress = true;
            ramNumberBox.Value = Math.Round(ramSlider.Value);
            SyncHints();
            syncInProgress = false;
        };

        cpuNumberBox.ValueChanged += (_, _) =>
        {
            if (syncInProgress)
            {
                return;
            ...
            syncInProgress = true;
            cpuSlider.Value = cpuNumberBox.Value;
            SyncHints();
            syncInProgress = false;
        };

        ramNumberBox.ValueChanged += (_, _) =>
        {
            if (syncInProgress)
            {
                return;
            ...
            syncInProgress = true;
            ramSlider.Value = ramNumberBox.Value;
            SyncHints();
            syncInProgress = false;
        };
        ...
        var cpuCard = new Border
        {
            ...
            Child = new StackPanel
            {
                ...
                Children =
                {
                    new TextBlock { Text = "CPU / Prozessor", Foreground = primaryTextBrush, FontSize = 15, FontWeight = Microsoft.UI.Text.FontWeights.SemiBold },
                    cpuHint,
                    cpuSlider,
                    cpuNumberBox
                }
        ...
        var ramCard = new Border
        {
            ...
            Child = new StackPanel
            {
                ...
                Children =
                {
                    new TextBlock { Text = "RAM / Arbeitsspeicher", Foreground = primaryTextBrush, FontSize = 15, FontWeight = Microsoft.UI.Text.FontWeights.SemiBold },
                    ramHint,
                    ramSlider,
                    ramNumberBox
                }
        ...
        var infoCard = new Border
        {
            ...
            Child = new StackPanel
            {
                ...
                Children =
                {
                    new TextBlock
                    {
                        Text = $"Ressourcen von '{vm.DisplayLabel}' anpassen",
                    ...
                    new TextBlock
                    {
                        Text = $"Host-Limits: bis {settings.HostLogicalProcessorCount} logische CPUs, {settings.HostTotalMemoryGb} GB RAM",
                        Foreground = mutedTextBrush,
                        FontSize = 12,
                        TextWrapping = TextWrapping.Wrap
                    }
        ...
        var content = new StackPanel { Spacing = 10, MinWidth = 500, MaxWidth = 540 };
        content.Children.Add(infoCard);
        content.Children.Add(cpuCard);
        ...
        var dialogScroll = new ScrollViewer
        {
        ...
        {
            XamlRoot = xamlRoot,
            Title = "CPU / RAM bearbeiten",
            Content = dialogScroll,
            PrimaryButtonText = "Übernehmen",
            CloseButtonText = "Abbrechen",
            DefaultButton = ContentDialogButton.Primary
        };
        dialog.RequestedTheme = string.Equals(_viewModel.UiTheme, "Dark", StringComparison.OrdinalIgnoreCase)
            ? ElementTheme.Dark
            : ElementTheme.Light;
        ...
        var selectedCpu = (int)Math.Round(cpuNumberBox.Value);
    ...
    private static bool IsVmStoppedForComputeChange(string? runtimeState)
    {
        ...
        return runtimeState.Contains("Off", StringComparison.OrdinalIgnoreCase)
               || runtimeState.Contains("Aus", StringComparison.OrdinalIgnoreCase)
               || runtimeState.Contains("Stopped", StringComparison.OrdinalIgnoreCase)
               || runtimeState.Contains("Heruntergefahren", StringComparison.OrdinalIgnoreCase)
               || runtimeState.Contains("Ausgeschaltet", StringComparison.OrdinalIgnoreCase);
    }

    private async Task MountIsoForVmAsync(VmDefinition vm)
    {
        if (vm is null || string.IsNullOrWhiteSpace(vm.Name))
        {
            return;
        ...
        var mounted = await _viewModel.MountIsoByVmNameAsync(vm.Name);
        if (mounted)
        {
            RequestVmChipsRefresh();
            UpdateIsoActionButtons();
    ...
    private async Task UnmountIsoForVmAsync(VmDefinition vm)
    {
        if (vm is null || string.IsNullOrWhiteSpace(vm.Name))
        {
            return;
        ...
        var unmounted = await _viewModel.UnmountIsoByVmNameAsync(vm.Name);
        if (unmounted)
        {
            RequestVmChipsRefresh();
            UpdateIsoActionButtons();
    ...
    private void UpdateIsoActionButtons()
    {
        if (_mountIsoButton is null || _unmountIsoButton is null)
        {
            return;
        }

        var selectedVm = _viewModel.SelectedVm;
        var hasMountedIso = selectedVm?.HasMountedIso ?? false;
        var hasSelection = selectedVm is not null;

        _mountIsoButton.Visibility = hasMountedIso ? Visibility.Collapsed : Visibility.Visible;
        _unmountIsoButton.Visibility = hasMountedIso ? Visibility.Visible : Visibility.Collapsed;

        _mountIsoButton.IsEnabled = hasSelection;
        _unmountIsoButton.IsEnabled = hasSelection;
    }
    ...
    private void OpenHelpWindow()
    {
        ...
        var repoUrl = "https://github.com/kaktools/HyperTool";
    ...
    private void HandleViewModelPropertyChangedOnUiThread(PropertyChangedEventArgs e)
    {
        ...
        if (string.Equals(e.PropertyName, nameof(MainViewModel.ResourceMonitorVersion), StringComparison.Ordinal))
        {
            if (_isMainLayoutLoaded)
            {
                RequestVmChipsRefresh();
    ...
    private Task OpenLatestGuestReleasePageAsync()
    {
        try
        {
            Process.Start(new ProcessStartInfo
            ...
            _viewModel.PublishNotification("Release-Seite für HyperTool Guest wurde geöffnet.", "Info");
        }
        catch (Exception ex)
        {
            _viewModel.PublishNotification($"Release-Seite für HyperTool Guest konnte nicht geöffnet werden: {ex.Message}", "Error");
        }
