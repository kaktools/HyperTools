...
public sealed partial class App : Application
{
    ...
    public static bool AreAllRequiredHyperVSocketServicesRegistered()
    {
        return GetMissingRequiredHyperVSocketServiceIds().Count == 0;
    }

    public static IReadOnlyList<string> GetMissingRequiredHyperVSocketServiceIds()
    {
        return GetMissingHyperVSocketServiceIds();
    }
    ...
    private void StartResourceMonitorListenerWithRecovery(bool isThemeRestart = false)
    {
        if (_mainViewModel is null)
        {
            return;
        ...
        try
        {
            ...
            Log.Information(isThemeRestart
                ? "Hyper-V socket resource monitor listener restarted after theme change."
                : "Hyper-V socket resource monitor listener started.");
        }
        catch (Exception ex)
        {
            _resourceMonitorHostListener?.Dispose();
    ...
    private void StartResourceMonitorLoop()
    {
        if (_mainViewModel is null)
        {
            return;
        ...
        _mainViewModel.ConfigureResourceMonitoring(
            _mainViewModel.MonitoringEnabled,
            _mainViewModel.MonitoringIntervalMs,
            _mainViewModel.MonitoringHistoryMinutes,
            _mainViewModel.MonitoringHistorySize);

        _resourceMonitorCts = new CancellationTokenSource();
        _resourceMonitorTask = Task.Run(() => RunResourceMonitorLoopAsync(_resourceMonitorCts.Token));
    }

    private void StopResourceMonitorLoop()
    {
        try
        {
            _resourceMonitorCts?.Cancel();
        }
        catch
        {
        }

        _resourceMonitorCts?.Dispose();
        _resourceMonitorCts = null;
        _resourceMonitorTask = null;
    }

    private async Task RunResourceMonitorLoopAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            try
            {
                if (_mainViewModel is null || !_mainViewModel.MonitoringEnabled)
                {
