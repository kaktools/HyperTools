...
public sealed class UsbIpDeviceInfo
{
    ...
    public string CustomLabelDisplay
    {
        get
        {
            if (!string.IsNullOrWhiteSpace(CustomName) && !string.IsNullOrWhiteSpace(CustomComment))
            {
                return $"{CustomName}  |  {CustomComment}";
            }
    ...
    public string CustomCommentDisplay => string.IsNullOrWhiteSpace(CustomComment)
        ? "-"
        : CustomComment;
