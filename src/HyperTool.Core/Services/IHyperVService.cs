...
public interface IHyperVService
{
    ...
    Task SetVmComputeSettingsAsync(string vmName, int cpuCount, int startupMemoryGb, CancellationToken cancellationToken);

    Task MountVmIsoAsync(string vmName, string isoPath, CancellationToken cancellationToken);

    Task UnmountVmIsoAsync(string vmName, CancellationToken cancellationToken);
