...
public sealed class HyperVPowerShellService : IHyperVService
{
        ...
        const string script = """
            @(
                Get-VM | ForEach-Object {
                    ...
                    $dvd = Get-VMDvdDrive -VMName $_.Name -ErrorAction SilentlyContinue | Select-Object -First 1
    ...
    public Task RenameVmAsync(string vmName, string newVmName, CancellationToken cancellationToken)
    {
        ...
        {
            throw new ArgumentException("Neuer VM-Name darf nicht leer sein.", nameof(newVmName));
        }

        return InvokeNonQueryAsync(
            $"Rename-VM -Name {ToPsSingleQuoted(vmName)} -NewName {ToPsSingleQuoted(newVmName)} -Confirm:$false",
            cancellationToken);
    }

    public async Task<VmComputeSettingsInfo> GetVmComputeSettingsAsync(string vmName, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(vmName))
        {
            throw new ArgumentException("VM-Name darf nicht leer sein.", nameof(vmName));
        ...
        var script =
            ...
            "$hostLogicalProcessors = 0; " +
            "if ($null -ne $vmHost -and $null -ne $vmHost.LogicalProcessorCount) { $hostLogicalProcessors = [int]$vmHost.LogicalProcessorCount }; " +
            "if ($hostLogicalProcessors -le 0) { $hostLogicalProcessors = [Environment]::ProcessorCount }; " +
            "$hostTotalMemoryBytes = 0L; " +
            "try { " +
            "  $computerSystem = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction Stop; " +
            "  if ($null -ne $computerSystem -and $null -ne $computerSystem.TotalPhysicalMemory) { $hostTotalMemoryBytes = [int64]$computerSystem.TotalPhysicalMemory } " +
            "} catch { }; " +
            "if ($hostTotalMemoryBytes -le 0) { $hostTotalMemoryBytes = [int64](8GB) }; " +
            "$hostTotalMemoryGb = [int][Math]::Floor($hostTotalMemoryBytes / 1GB); " +
            "if ($hostTotalMemoryGb -lt 1) { $hostTotalMemoryGb = 1 }; " +
            "$recommendedMaxMemoryGb = [int][Math]::Floor(($hostTotalMemoryGb * 0.9)); " +
            "if ($recommendedMaxMemoryGb -lt 1) { $recommendedMaxMemoryGb = 1 }; " +
            "if ($recommendedMaxMemoryGb -gt $hostTotalMemoryGb) { $recommendedMaxMemoryGb = $hostTotalMemoryGb }; " +
            "$cpuCount = [int]$vmProcessor.Count; " +
            "if ($cpuCount -lt 1) { $cpuCount = 1 }; " +
            "$startupMemoryGb = [int][Math]::Round(([double]$vmMemory.Startup / 1GB), 0, [System.MidpointRounding]::AwayFromZero); " +
            "if ($startupMemoryGb -lt 1) { $startupMemoryGb = 1 }; " +
            "$result = [pscustomobject]@{ " +
            "  CpuCount = $cpuCount; " +
            "  StartupMemoryGb = $startupMemoryGb; " +
            "  DynamicMemoryEnabled = [bool]$vmMemory.DynamicMemoryEnabled; " +
            "  MinCpuCount = 1; " +
            "  MaxCpuCount = [int][Math]::Max(1, $hostLogicalProcessors); " +
            "  MinStartupMemoryGb = 1; " +
            "  MaxStartupMemoryGb = [int][Math]::Max(1, $recommendedMaxMemoryGb); " +
            "  HostLogicalProcessorCount = [int][Math]::Max(1, $hostLogicalProcessors); " +
            "  HostTotalMemoryGb = [int][Math]::Max(1, $hostTotalMemoryGb) " +
            "}; " +
        ...
        {
            return new VmComputeSettingsInfo();
        }

        var minCpu = (int)Math.Max(1, GetInt64(row, "MinCpuCount"));
        var maxCpu = (int)Math.Max(minCpu, GetInt64(row, "MaxCpuCount"));
        var minRam = (int)Math.Max(1, GetInt64(row, "MinStartupMemoryGb"));
        var maxRam = (int)Math.Max(minRam, GetInt64(row, "MaxStartupMemoryGb"));

        return new VmComputeSettingsInfo
        {
            CpuCount = (int)Math.Clamp(GetInt64(row, "CpuCount"), minCpu, maxCpu),
            StartupMemoryGb = (int)Math.Clamp(GetInt64(row, "StartupMemoryGb"), minRam, maxRam),
            DynamicMemoryEnabled = GetBoolean(row, "DynamicMemoryEnabled"),
            MinCpuCount = minCpu,
            MaxCpuCount = maxCpu,
            MinStartupMemoryGb = minRam,
            MaxStartupMemoryGb = maxRam,
            HostLogicalProcessorCount = (int)Math.Max(1, GetInt64(row, "HostLogicalProcessorCount")),
            HostTotalMemoryGb = (int)Math.Max(1, GetInt64(row, "HostTotalMemoryGb"))
        };
    }

    public async Task SetVmComputeSettingsAsync(string vmName, int cpuCount, int startupMemoryGb, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(vmName))
        {
            throw new ArgumentException("VM-Name darf nicht leer sein.", nameof(vmName));
        ...
        var normalizedStartupMemoryGb = Math.Max(1, startupMemoryGb);

        var script =
            "$vmName = " + ToPsSingleQuoted(vmName) + "; " +
            "$cpuCount = " + normalizedCpu.ToString(CultureInfo.InvariantCulture) + "; " +
            "$startupBytes = " + normalizedStartupMemoryGb.ToString(CultureInfo.InvariantCulture) + "GB; " +
            "Set-VMProcessor -VMName $vmName -Count $cpuCount -ErrorAction Stop; " +
            "Set-VMMemory -VMName $vmName -StartupBytes $startupBytes -ErrorAction Stop";
        ...
        {
            await InvokeElevatedPowerShellAsync(script, "CPU/RAM-Änderung", cancellationToken);
        }
    }

    public async Task MountVmIsoAsync(string vmName, string isoPath, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(vmName))
        {
            throw new ArgumentException("VM-Name darf nicht leer sein.", nameof(vmName));
        ...
        {
            throw new ArgumentException("ISO-Pfad darf nicht leer sein.", nameof(isoPath));
        }

        var preparedIsoPath = PrepareIsoPathForHyperV(isoPath);

        var script =
            "$vmName = " + ToPsSingleQuoted(vmName) + "; " +
            "$isoPath = " + ToPsSingleQuoted(preparedIsoPath) + "; " +
            "if (-not (Test-Path -LiteralPath $isoPath -PathType Leaf)) { throw 'ISO-Datei wurde nicht gefunden.' }; " +
            ...
            "} else { " +
            "  Set-VMDvdDrive -VMName $vmName -ControllerNumber $dvd.ControllerNumber -ControllerLocation $dvd.ControllerLocation -Path $isoPath -ErrorAction Stop | Out-Null; " +
            "}";

        await MountVmIsoWithElevationFallbackAsync(script, cancellationToken);
    }

    public async Task UnmountVmIsoAsync(string vmName, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(vmName))
        {
            throw new ArgumentException("VM-Name darf nicht leer sein.", nameof(vmName));
        ...
        var script =
            "$vmName = " + ToPsSingleQuoted(vmName) + "; " +
            "$dvd = Get-VMDvdDrive -VMName $vmName -ErrorAction SilentlyContinue | Select-Object -First 1; " +
            "if ($null -eq $dvd) { return }; " +
            "Set-VMDvdDrive -VMName $vmName -ControllerNumber $dvd.ControllerNumber -ControllerLocation $dvd.ControllerLocation -Path $null -ErrorAction Stop | Out-Null";
        ...
        {
            await InvokeElevatedPowerShellAsync(script, "ISO auswerfen", cancellationToken);
        }
    }

    private static string PrepareIsoPathForHyperV(string isoPath)
    {
        var normalized = (isoPath ?? string.Empty).Trim();
        ...
        {
            throw new FileNotFoundException("ISO-Datei wurde nicht gefunden.", normalized);
        }
        ...
        try
        {
            var sourceInfo = new FileInfo(normalized);
            var fileName = sourceInfo.Name;
            if (string.IsNullOrWhiteSpace(fileName))
            {
                throw new InvalidOperationException("Dateiname aus UNC-ISO-Pfad konnte nicht ermittelt werden.");
            }
            ...
            var hashInput = $"{sourceInfo.FullName}|{sourceInfo.Length}|{sourceInfo.LastWriteTimeUtc.Ticks}";
            var hashBytes = SHA256.HashData(Encoding.UTF8.GetBytes(hashInput));
            var hash = Convert.ToHexString(hashBytes)[..12];

            var safeBaseName = Path.GetFileNameWithoutExtension(fileName);
            var extension = Path.GetExtension(fileName);
            var cachedFileName = string.IsNullOrWhiteSpace(extension)
                ? $"{safeBaseName}_{hash}"
                : $"{safeBaseName}_{hash}{extension}";
            var cachedIsoPath = Path.Combine(cacheRoot, cachedFileName);

            if (!File.Exists(cachedIsoPath))
            {
                File.Copy(sourceInfo.FullName, cachedIsoPath, overwrite: false);
            }

            return cachedIsoPath;
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException(
                "ISO liegt auf einer Netzwerkfreigabe und konnte nicht lokal zwischengespeichert werden. " +
                "Bitte ISO lokal auf dem Host ablegen oder Freigabe-/NTFS-Rechte pruefen.",
                ex);
        }
    }
    ...
    private async Task MountVmIsoWithElevationFallbackAsync(string script, CancellationToken cancellationToken)
    {
        try
        {
            await InvokeNonQueryAsync(script, cancellationToken);
        ...
        {
            // If Hyper-V rejects the current token, retry once via elevated PowerShell (UAC prompt).
            await InvokeElevatedPowerShellAsync(script, "ISO-Einbindung", cancellationToken);
        }
    }

    private static async Task InvokeElevatedPowerShellAsync(string script, string operationName, CancellationToken cancellationToken)
    {
        string? tempScriptPath = null;
        string? tempErrorPath = null;
        try
        {
            var safeOperationName = string.IsNullOrWhiteSpace(operationName) ? "Aktion" : operationName;
            tempScriptPath = Path.Combine(Path.GetTempPath(), $"HyperTool.Elevated.{Guid.NewGuid():N}.ps1");
            tempErrorPath = Path.Combine(Path.GetTempPath(), $"HyperTool.Elevated.{Guid.NewGuid():N}.error.txt");

            var escapedErrorPath = (tempErrorPath ?? string.Empty).Replace("'", "''");
            var scriptBuilder = new StringBuilder();
            scriptBuilder.AppendLine("$ErrorActionPreference = 'Stop'");
            scriptBuilder.AppendLine("try {");
            scriptBuilder.AppendLine("  Import-Module Hyper-V -ErrorAction Stop");
            scriptBuilder.AppendLine("  " + script);
            scriptBuilder.AppendLine("  exit 0");
            scriptBuilder.AppendLine("} catch {");
            scriptBuilder.AppendLine($"  $_ | Out-File -LiteralPath '{escapedErrorPath}' -Encoding utf8 -Force");
            scriptBuilder.AppendLine($"  if ($_.InvocationInfo -and $_.InvocationInfo.PositionMessage) {{ $_.InvocationInfo.PositionMessage | Out-File -LiteralPath '{escapedErrorPath}' -Encoding utf8 -Append }}");
            scriptBuilder.AppendLine("  exit 1");
            scriptBuilder.AppendLine("}");

            File.WriteAllText(tempScriptPath, scriptBuilder.ToString(), new UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
            ...
            if (process.ExitCode != 0)
            {
                ...
                if (!string.IsNullOrWhiteSpace(tempErrorPath) && File.Exists(tempErrorPath))
                {
                    errorDetails = File.ReadAllText(tempErrorPath).Trim();
                }

                if (string.IsNullOrWhiteSpace(errorDetails))
                {
                    throw new InvalidOperationException($"{safeOperationName} mit Adminrechten fehlgeschlagen (ExitCode {process.ExitCode}).");
                }

                throw new InvalidOperationException($"{safeOperationName} mit Adminrechten fehlgeschlagen: {errorDetails}");
            }
        }
        catch (Win32Exception ex) when (ex.NativeErrorCode == 1223)
        {
            var safeOperationName = string.IsNullOrWhiteSpace(operationName) ? "Aktion" : operationName;
            throw new UnauthorizedAccessException($"{safeOperationName} abgebrochen: UAC-Bestätigung wurde abgelehnt.");
        }
        finally
        {
            try
    ...
    public async Task<IReadOnlyList<HyperVVmNetworkAdapterInfo>> GetVmNetworkAdaptersAsync(string vmName, CancellationToken cancellationToken)
    {
        var script =
            ...
            "IpAddresses = @($_.IPAddresses | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }) } }) | ConvertTo-Json -Depth 6 -Compress";
        ...
        CancellationToken cancellationToken)
    {
        var script = $"$importPath = {ToPsSingleQuoted(importPath)}; " +
                     ...
                     "$switchFallbackMode = 'none'; " +
                     "$switchFallbackAdjustedAdapters = 0; " +
                     ...
                     "$compareReport = Compare-VM -Path $configPath -Copy -GenerateNewId -VirtualMachinePath $virtualMachinePath -VhdDestinationPath $vhdPath -SnapshotFilePath $snapshotPath -SmartPagingFilePath $smartPagingPath -ErrorAction Stop; " +
                     "} elseif ($importMode -eq 'register') { " +
                     "$compareReport = Compare-VM -Path $configPath -Register -ErrorAction Stop; " +
                     "} else { " +
                     "$compareReport = Compare-VM -Path $configPath -ErrorAction Stop; " +
                     "}; " +
                     "if ($null -eq $compareReport) { throw 'Kompatibilitätsprüfung für VM-Import fehlgeschlagen.' }; " +
                     "$defaultSwitch = Get-VMSwitch -ErrorAction SilentlyContinue | Where-Object { $_.Name -ceq 'Default Switch' } | Select-Object -First 1; " +
                     "$adapterSources = @(); " +
                     "if ($null -ne $compareReport.Incompatibilities) { " +
                     "$adapterSources = @($compareReport.Incompatibilities | ForEach-Object { if ($null -ne $_.Source -and $_.Source.PSObject.TypeNames -contains 'Microsoft.HyperV.PowerShell.VMNetworkAdapter') { $_.Source } }) " +
                     "}; " +
                     "$adapterSources = @($adapterSources | Sort-Object Id -Unique); " +
                     "foreach ($adapter in $adapterSources) { " +
                     "if ($null -eq $adapter) { continue }; " +
                     "try { " +
                     "if ($null -ne $defaultSwitch -and -not [string]::IsNullOrWhiteSpace($defaultSwitch.Name)) { " +
                     "Connect-VMNetworkAdapter -VMNetworkAdapter $adapter -SwitchName $defaultSwitch.Name -ErrorAction Stop; " +
                     "$switchFallbackMode = 'default'; " +
                     "} else { " +
                     "Disconnect-VMNetworkAdapter -VMNetworkAdapter $adapter -ErrorAction Stop; " +
                     "if ($switchFallbackMode -ne 'default') { $switchFallbackMode = 'disconnected' }; " +
                     "}; " +
                     "$switchFallbackAdjustedAdapters++; " +
                     "} catch { " +
                     "if ($switchFallbackMode -eq 'none') { $switchFallbackMode = 'unresolved' }; " +
                     "} " +
                     "}; " +
                     "$job = Import-VM -CompatibilityReport $compareReport -Confirm:$false -AsJob; " +
                     ...
                     "Write-Output ('HT_SWITCH_MODE:' + $switchFallbackMode); " +
                     "Write-Output ('HT_SWITCH_ADJUSTED:' + $switchFallbackAdjustedAdapters)";
        ...
        var switchModeResult = GetResultToken(output, "HT_SWITCH_MODE:");
        var switchAdjustedResult = GetResultToken(output, "HT_SWITCH_ADJUSTED:");
        ...
        return new ImportVmResult
        {
            ...
            SwitchFallbackMode = string.IsNullOrWhiteSpace(switchModeResult) ? string.Empty : switchModeResult["HT_SWITCH_MODE:".Length..].Trim(),
            SwitchFallbackAdjustedAdapters = string.IsNullOrWhiteSpace(switchAdjustedResult)
                ? 0
                : int.TryParse(switchAdjustedResult["HT_SWITCH_ADJUSTED:".Length..].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsedAdjusted)
                    ? Math.Max(parsedAdjusted, 0)
                    : 0
        };
    }
    ...
    private static async Task<string> InvokePowerShellAsync(string script, CancellationToken cancellationToken)
    {
        ...
        if (process.ExitCode != 0)
        {
            ...
            {
                var elevatedHint = IsProcessElevated()
                    ? " Der Prozess läuft bereits erhöht. Prüfe Importpfad, Dateizugriffe und Hyper-V-Konfiguration."
                    : " Bitte HyperTool als Administrator starten oder den Benutzer zur Gruppe 'Hyper-V-Administratoren' hinzufügen.";
                throw new UnauthorizedAccessException(
                    "Keine Berechtigung für Hyper-V." + elevatedHint);
            }
    ...
    private static List<string> GetStringList(JsonElement row, string propertyName)
    {
        if (!row.TryGetProperty(propertyName, out var element))
        {
            return [];
        }
