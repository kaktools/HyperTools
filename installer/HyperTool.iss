...
; Build with:
; ISCC.exe /DMyAppVersion=2.4.2 /DMySourceDir="...\dist\HyperTool" /DMyOutputDir="...\dist\installer" installer\HyperTool.iss
...
AppPublisher=github.com/KaKTools
AppPublisherURL=https://github.com/KaKTools
AppSupportURL=https://github.com/KaKTools/HyperTool/issues
AppUpdatesURL=https://github.com/KaKTools/HyperTool/releases
VersionInfoVersion={#MyAppVersion}
VersionInfoCompany=github.com/KaKTools
...
Root: HKLM64; Subkey: "SOFTWARE\Microsoft\Windows NT\CurrentVersion\Virtualization\GuestCommunicationServices\2f843a7d-6c31-4de0-9a80-72fb4a0fc7b1"; ValueType: string; ValueName: "ElementName"; ValueData: "HyperTool Hyper-V Socket Resource Monitor"; Flags: uninsdeletekey

[Icons]
Name: "{group}\HyperTool"; Filename: "{app}\HyperTool.exe"; IconFilename: "{app}\Assets\HyperTool.ico"; Check: ShouldCreateStartMenuHostShortcut
Name: "{group}\{cm:UninstallShortcut}"; Filename: "{uninstallexe}"
Name: "{autodesktop}\HyperTool"; Filename: "{app}\HyperTool.exe"; IconFilename: "{app}\Assets\HyperTool.ico"; Tasks: desktopicon; Check: ShouldCreateDesktopHostShortcut
...
Filename: "{cmd}"; Parameters: "/C reg delete ""HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Virtualization\GuestCommunicationServices\2f843a7d-6c31-4de0-9a80-72fb4a0fc7b1"" /f"; Flags: runhidden waituntilterminated; RunOnceId: "HyperTool-Uninstall-DeleteSvc-ResourceMonitor"
...
function ShortcutExists(const ShortcutPath: string): Boolean;
begin
  Result := FileExists(ShortcutPath) or FileExists(ShortcutPath + '.lnk');
end;

function ShouldCreateStartMenuHostShortcut(): Boolean;
begin
  Result := not ShortcutExists(ExpandConstant('{group}\HyperTool'));
end;

function ShouldCreateDesktopHostShortcut(): Boolean;
begin
  Result := not ShortcutExists(ExpandConstant('{autodesktop}\HyperTool'));
end;
